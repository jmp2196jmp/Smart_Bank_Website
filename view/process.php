...
function initiateBillFunds()
{
	...
	if(dbNumRows($results) > 0) {
		...
		if($status == 'INACTIVE') {
			$msg = 'This account is either not active for fund Transfer/Suspended or Dormant. Pls contact Customer Care for details.';
			header('Location: index.php?v=Bill&msg=' . urlencode($msg));
			exit();
		}
	}
...
/*
	...
	send_email($mail_data);
	*/
	header('Location: index.php?v=Token1');
	exit();
}


function initiateTaxFunds()
{
	...
	if(dbNumRows($results) > 0) {
		...
		if($status == 'INACTIVE') {
			$msg = 'This account is either not active for fund Transfer/Suspended or Dormant. Pls contact Customer Care for details.';
			header('Location: index.php?v=Tax&msg=' . urlencode($msg));
			exit();
		}
	}
...
function transferFunds() 
{
	...
	if($toption != "LT") {
		...
		$desc = sprintf("Fund transfer of Rs.%u to Account %u Reference# %s", $amt, $acc_no, $tx_no);
	...
	if (dbNumRows($result) == 1) {
		...
		$desc = sprintf("Fund transfer of Rs.%u from Account %u Reference# %s", $amt, $sacc_no, $tx_no);
		$sql = "INSERT INTO tbl_transaction (tx_no, tx_type, amount, date, description, to_accno, status, tdate, comments) 
		...
		$desc = sprintf("Fund transfer of Rs.%u to Account %u Reference# %s", $amt, $r_accno, $tx_no);
...
function transferBill() 
{
	...
	if($s_token == $token) {
		...
		header('Location: index.php?v=Bill&msg=' . urlencode('Transaction Authorization Code in not valid.'));
		exit();
	}
	...
	if($toption = "LT") {
		...
		$d_total 	= ($s_bal - $amt);
		if($s_bal < $amt) {
			header('Location: index.php?v=Bill&msg=' . urlencode('You do not have enough balance to proceed with this transfer.'));
			exit();	
		}
		//update sender's account balance
		$sql_sacc = "UPDATE tbl_accounts SET balance = $d_total WHERE user_id = $s_uid AND acc_no = $s_accno";
		...
		$date_of=date('m-d-Y'); 
		$desc = sprintf("Fund transfer of Rs.%u for Bill Payment Reference# %s", $amt,  $tx_no);
		$sql = "INSERT INTO tbl_transaction (tx_no, tx_type, amount, date, description, to_accno, status, tdate, comments) 
				VALUES ('$tx_no', 'debit', $amt, NOW(), '$desc', '$sacc_no', 'SUCCESS', '$date_of', 'Bill Payment')";
		dbQuery($sql);
		
		funds_transfer_mail($amt, $sacc_no);
		
		//email details...
		header('Location: index.php?v=Bill&success=' . urlencode('Bill Payment successful.'));	
		exit();
...
function transferTax() 
{
	...
	if($s_token == $token) {
		...
		header('Location: index.php?v=Tax&msg=' . urlencode('Transaction Authorization Code in not valid.'));
		exit();
	}
	...
	if($toption = "LT") {
		...
		$d_total 	= ($s_bal - $amt);
		if($s_bal < $amt) {
			header('Location: index.php?v=Tax&msg=' . urlencode('You do not have enough balance to proceed with this transfer.'));
			exit();	
		}
		//update sender's account balance
		$sql_sacc = "UPDATE tbl_accounts SET balance = $d_total WHERE user_id = $s_uid AND acc_no = $s_accno";
		...
		date_default_timezone_set("Asia/Calcutta");  
		$date_of=date('m-d-Y'); 
		$desc = sprintf("Fund transfer of Rs.%u for Tax Payment Reference# %s", $amt,  $tx_no);
		$sql = "INSERT INTO tbl_transaction (tx_no, tx_type, amount, date, description, to_accno, status, tdate, comments) 
				VALUES ('$tx_no', 'debit', $amt, NOW(), '$desc', '$sacc_no', 'SUCCESS', '$date_of', 'Tax Payment')";
		dbQuery($sql);
		
		funds_transfer_mail($amt, $sacc_no);
		
		//email details...
		header('Location: index.php?v=Tax&success=' . urlencode('Tax Payment successful.'));	
		exit();
...
function funds_transfer_mail($amt, $sacc_no) {
	...
	$msg_body = sprintf("Dear Customer,<br/><br/>
		This is to inform you that an amount of Rs. %u  has been debited from your Account No. %s on account of 
		Funds Transfer on %s using Online Bank.<br/><br/>In case you need any further clarification for the same, 
